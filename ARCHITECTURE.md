...
실호출 결과에서 그대로 옮겼다. (2026-09-03 시점의 `PROJECT_STRUCTURE.md` 를 대체한다 —
캠핑·핫플레이스·커뮤니티가 빠져 있고 쓰지 않는 API 가 적혀 있어 지웠다.
필요하면 `git show 523c623:PROJECT_STRUCTURE.md` 로 볼 수 있다.)
...
`petTourSyncList2` 가 `lclsSystm1/2/3` 을 코드로만 주므로 이름을 따로 받는다.
